...
# print(f">>>>>>>> Loggers list: {loggers_dict}")
...
class FacebookAutomation:
    ...
    def get_messenger(self, timeout, retries):
        messenger = try_and_wait(self.driver.find_element, [By.XPATH, "//a[contains(@aria-label, 'Messenger')]"], timeout, retries)
        if messenger:
            return messenger
        return False
            ...
            if trust_this_device:
                ...
                if self.is_home(2, 30):
                    if self.is_inbox(2, 30):
                        messenger = self.get_messenger(2, 5)
                        if messenger:
                            self.driver.execute_script("arguments[0].click();", messenger)
                            with sessions_lock:
                ...
                try:
                    ...
                    if is_new_message:
                        ...
                        if response:
                            ...
                            text_area = try_and_wait(self.driver.find_element, [By.XPATH, "//textarea[@placeholder='Reply in Messenger…']"], 1, 30)
    ...
    def track_message_recive(self, recieve_from, session_name, message_text):
        ...
        url = f"{os.environ.get('FACEBOOK_AI_TRACKING_URL')}/receive"
        try:
    ...
    def track_message_sent(self, sent_to, session_name, bot_response):
        ...
        url = f"{os.environ.get('FACEBOOK_AI_TRACKING_URL')}/sent"
        try:
            requests.request("POST", url, data=payload)
        ...
        with sessions_lock:   
            for session_ in sessions:
                if stored_session_name in session_:
                    ...
                    obj = FacebookAutomation(**session_data)
...
# @auth_required
def home():
    return {"Message": "Facebook AI is ready to fly."}


@app.route("/api/face/sessions/start", methods=["POST"])
@auth_required
...
@app.route("/api/face/sessions", methods=["GET"])
@auth_required
def return_sessions():
    try:
        sessions_data = []
...
@app.route("/api/face/status/session", methods=["GET"])
@auth_required
...
@app.route("/api/face/screenshot", methods=["GET"])
@auth_required
...
@app.route("/api/face/sessions/stop", methods=["POST"])
@auth_required
...
@app.route("/api/face/sessions/logout", methods=["POST"])
@auth_required
...
@app.route("/api/face/2fa/code", methods=["POST"])
@auth_required
def code_auth():
    try:
...
@app.route("/api/face/threads", methods=["GET"])
@auth_required
def return_threads():
